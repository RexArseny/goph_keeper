...
type TextsData struct {
	...
	Texts      map[int]models.Text
	Editing    bool
	CurrentRow int
	CurrentCol int
}

func (d *TextsData) Done(key tcell.Key) {
	if key == tcell.KeyEnter {
		...
		d.Texts[d.CurrentRow] = models.Text{
			ID:   id,
			Text: d.Table.GetCell(d.CurrentRow, 1).Text,
		}
	}
	d.Editing = false
...
func (d *TextsData) Selected(row int, column int) {
